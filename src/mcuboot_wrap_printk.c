...
static void
vprintk_to_segger_rtt(const char* fmt, va_list ap)
{
    char log_buffer[SEGGER_RTT_WRAP_LOG_BUFFER_SIZE];

    int length = vsnprintf(log_buffer, sizeof(log_buffer), fmt, ap);
    ...
    if (length >= sizeof(log_buffer))
    {
        ...
        length                             = (int)sizeof(log_buffer) - 1;
    }

    if (length > 0)
    {
        mcuboot_segger_rtt_write(log_buffer, (unsigned)length);
    }
}
#endif // (defined(CONFIG_USE_SEGGER_RTT) && defined(CONFIG_RTT_CONSOLE)) && (defined(CONFIG_SERIAL) &&
       // defined(CONFIG_UART_CONSOLE))

__printf_like(1, 0) void __wrap_vprintk(const char* fmt, va_list ap)
{
    ...
    extern __printf_like(1, 0) void __real_vprintk(const char* fmt, va_list ap);
