    ...
    int                     seed_len)
{
    ...
    for (off = 0; off < size; off += blk_sz)
    {
        ...
        rc = load_image_data(p_file, off, tmp_buf, blk_sz);
        if (rc)
        {
...
#if !defined(MCUBOOT_HW_KEY)
static int
file_img_find_key(uint8_t* keyhash, uint8_t keyhash_len)
{
    bootutil_sha_context       sha_ctx;
    int                        i;
 ...
 */
int32_t
file_img_get_security_cnt(struct image_header* hdr, struct fs_file_t* const p_file, uint32_t* img_security_cnt)
{
    ...
    if ((hdr == NULL) || (p_file == NULL) || (img_security_cnt == NULL))
    {
        /* Invalid parameter. */
    ...
    rc = file_tlv_iter_begin(&it, hdr, p_file, IMAGE_TLV_SEC_CNT, true);
    if (rc)
    {
        return rc;
    }
...
file_img_check_for_pure(const struct image_header* hdr, struct fs_file_t* const p_file)
{
    ...
    rc = file_tlv_iter_begin(&it, hdr, fap, IMAGE_TLV_SIG_PURE, false);
    if (rc)
    {
        return rc;
    }
    ...
    uint8_t*                out_hash)
{
    ...
    if (IS_COMPRESSED(hdr))
    {
        ...
        rc = file_tlv_iter_begin(&it, hdr, fap, IMAGE_TLV_ANY, true);
        if (rc)
        {
            goto out;
        }
        ...
        while (true)
        {
            ...
            rc = file_tlv_iter_next(&it, &off, &len, &type);
            if (rc < 0)
            {
                goto out;
            }
    ...
    while (true)
    {
         ...
         * unprotected area.
         */
        if (!file_tlv_iter_is_prot(&it, off))
        {
            bool found = false;
        ...
        switch (type)
        {
            ...
            case EXPECTED_HASH_TLV:
            {
                ...
                rc = LOAD_IMAGE_DATA(hdr, p_file, off, buf, sizeof(hash));
                if (rc)
                {
                    goto out;
                }
            ...
            case EXPECTED_KEY_TLV:
            {
                ...
                rc = LOAD_IMAGE_DATA(hdr, p_file, off, buf, len);
                if (rc)
                {
                    goto out;
                }
                key_id = file_img_find_key(buf, len);
#else
                ...
                key_id = file_img_find_key(image_index, key_buf, len);
#endif /* !MCUBOOT_HW_KEY */
                /*
                 * The key may not be found, which is acceptable.  There
                 * can be multiple signatures, each preceded by a key.
                 */
                LOG_INF("EXPECTED_KEY_TLV: key_id=%d", key_id);
                break;
            }
            ...
            case EXPECTED_SIG_TLV:
            {
                ...
                LOG_INF(
                    ...
                    FIH_EQ(valid_signature, FIH_SUCCESS) ? "OK" : "FAIL");
                break;
            }
#endif /* EXPECTED_SIG_TLV */
#ifdef MCUBOOT_HW_ROLLBACK_PROT
            case IMAGE_TLV_SEC_CNT:
            {
                 ...
                 */
                fih_rc = fih_ret_encode_zero_equality(img_security_cnt < (uint32_t)fih_int_decode(security_cnt));
                if (FIH_NOT_EQ(fih_rc, FIH_SUCCESS))
                {
                    FIH_SET(fih_rc, FIH_FAILURE);
                    goto out;
     ...
     */
    if (!rc && IS_COMPRESSED(hdr))
    {
        image_hash_valid = 0;
        FIH_SET(valid_signature, FIH_FAILURE);

        rc = file_img_hash_decompress(enc_state, image_index, hdr, fap, tmp_buf, tmp_buf_sz, hash, seed, seed_len);
        if (rc)
        {
            goto out;
        }

        rc = file_tlv_iter_begin(&it, hdr, fap, IMAGE_TLV_DECOMP_SHA, true);
        if (rc)
        {
            goto out;
        }
...
#ifdef EXPECTED_SIG_TLV
#ifdef EXPECTED_KEY_TLV
        rc = file_tlv_iter_begin(&it, hdr, fap, EXPECTED_KEY_TLV, false);
        if (rc)
        {
            goto out;
        }
        ...
        rc = file_tlv_iter_begin(&it, hdr, fap, IMAGE_TLV_DECOMP_SIGNATURE, true);
        if (rc)
        {
            goto out;
        }
