...
#if ( \
    defined(MCUBOOT_OVERWRITE_ONLY) + defined(MCUBOOT_SWAP_USING_MOVE) + defined(MCUBOOT_DIRECT_XIP) \
...
#if !defined(MCUBOOT_OVERWRITE_ONLY) && !defined(MCUBOOT_SWAP_USING_MOVE) && !defined(MCUBOOT_DIRECT_XIP) \
    && !defined(MCUBOOT_RAM_LOAD) && !defined(MCUBOOT_SINGLE_APPLICATION_SLOT) && !defined(MCUBOOT_FIRMWARE_LOADER)
...
fih_ret
bootutil_verify_img(const uint8_t* img, uint32_t size, uint8_t* sig, size_t slen, uint8_t key_id);
 ...
 */
static inline bool
boot_u32_safe_add(uint32_t* dest, uint32_t a, uint32_t b)
{
    /*
 ...
 */
static inline bool
boot_u16_safe_add(uint16_t* dest, uint16_t a, uint16_t b)
{
    uint32_t tmp = a + b;
    if (tmp > UINT16_MAX)
    {
...
uint32_t
bootutil_max_image_size(const struct flash_area* fap);
